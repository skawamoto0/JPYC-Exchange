...
contract JpycExchangeV2 {
    ...
    function rateCombinedJpycV1ToJpycV2(uint256 route, uint256 amount) internal returns(uint256) {
        ...
        if(route == 1) {
            return rateJpycV1ToJpycV2(1, amount);
        }
        if(route == 2) {
    ...
    function exchangeCombinedJpycV1ToJpycV2(uint256 route, uint256 amount) internal {
        ...
        if(route == 1) {
            exchangeJpycV1ToJpycV2(1, amount);
            return;
        }
        if(route == 2) {
    ...
    function rateCombinedJpycV2ToJpycV1(uint256 route, uint256 amount) internal returns(uint256) {
        if(route == 0) {
            return rateJpycV2ToJpycV1(0, amount);
        }
        if(route == 1) {
            return rateJpycV2ToJpycV1(1, amount);
        }
        if(route == 2) {
    ...
    function exchangeCombinedJpycV2ToJpycV1(uint256 route, uint256 amount) internal {
        if(route == 0) {
            exchangeJpycV2ToJpycV1(0, amount);
            return;
        }
        if(route == 1) {
            exchangeJpycV2ToJpycV1(1, amount);
            return;
        }
        if(route == 2) {
    ...
    function rateCombinedUsdcToJpycV1(uint256 route, uint256 amount) internal returns(uint256) {
        ...
        if(route == 2) {
            return rateJpycV2ToJpycV1(1, rateUsdcToJpycV2(1, amount));
        }
        if(route == 3) {
    ...
    function exchangeCombinedUsdcToJpycV1(uint256 route, uint256 amount) internal {
        ...
        if(route == 2) {
            ...
            exchangeJpycV2ToJpycV1(1, jpycv2.balanceOf(address(this)));
            return;
        }
        if(route == 3) {
    ...
    function rateCombinedJpycV2ToUsdc(uint256 route, uint256 amount) internal returns(uint256) {
        ...
        if(route == 2) {
            return rateJpycV1ToUsdc(1, rateJpycV2ToJpycV1(1, amount));
        }
        if(route == 3) {
    ...
    function exchangeCombinedJpycV2ToUsdc(uint256 route, uint256 amount) internal {
        ...
        if(route == 2) {
            exchangeJpycV2ToJpycV1(1, amount);
            exchangeJpycV1ToUsdc(1, jpycv1.balanceOf(address(this)));
            return;
        }
        if(route == 3) {
            exchangeJpycV2ToJjpy(0, amount);
    ...
    function rateJpycV1ToJpycV2(uint256 route, uint256 amount) internal returns(uint256) {
        ...
        if(route == 1) {
            try quoterUniswap.quoteExactInputSingle(address(jpycv1), address(jpycv2), 500, amount, 0) returns(uint256 a) {
                return a;
            }
            catch {
            }
    ...
    function exchangeJpycV1ToJpycV2(uint256 route, uint256 amount) internal {
        ...
        if(route == 1) {
            routerUniswap.exactInputSingle(UniswapExactInputSingle(address(jpycv1), address(jpycv2), 500, address(this), block.timestamp, amount, 0, 0));
            return;
        }
        revert();
    }
    function rateJpycV2ToJpycV1(uint256 route, uint256 amount) internal returns(uint256) {
        if(amount == 0) {
            return 0;
        }
        if(route == 0) {
            try routerQuickSwap.getAmountsOut(amount, addressArray(address(jpycv2), address(jpycv1))) returns(uint256[] memory a) {
                return a[1];
            }
            catch {
            }
        ...
        if(route == 1) {
            try quoterUniswap.quoteExactInputSingle(address(jpycv2), address(jpycv1), 500, amount, 0) returns(uint256 a) {
                return a;
            }
            catch {
            }
    ...
    function exchangeJpycV2ToJpycV1(uint256 route, uint256 amount) internal {
        if(route == 0) {
            routerQuickSwap.swapExactTokensForTokens(amount, 0, addressArray(address(jpycv2), address(jpycv1)), address(this), block.timestamp);
            return;
        }
        if(route == 1) {
            routerUniswap.exactInputSingle(UniswapExactInputSingle(address(jpycv2), address(jpycv1), 500, address(this), block.timestamp, amount, 0, 0));
            return;
        }
        revert();
    }
